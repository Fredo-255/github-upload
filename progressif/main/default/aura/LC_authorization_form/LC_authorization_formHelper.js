({
    ...
    selectFromHeaderStep4 : function(component,event,helper){
     ...
     DisplayCaneaux : function(component, event, helper){
                     ...
                     function(response){
             			...
             			if(state === 'SUCCESS'){
                            ...
                            for(var i=0; i<result.length; i++){
                               ...
                               //Format Crit
                                 if (result[i].criterion__r.Cible__c=='Espace de diffusion' ){
                            ...
                            for (var i=0; i<result.length; i++){
                                ...
                                if (result[i].criterion__r.Cible__c=='Moyen de diffusion' ){
                                    var crit= this.CritFormat(result[i]);
                                    newmoyen.push(crit);
                                }
                            }
 	 ...
 	 DisplayCriteres : function (component, event, helper) {
         ...
         console.log('STEP 2 - OFFRE SELECTIONEE  = '+ OfferId);
         ...
         action.setCallback(this, function(response){
             ...
             if(state === 'SUCCESS'){
                 ...
                 console.log('STEP 2 - LISTE CRITERES OFFRE  =  ' + JSON.stringify(result));
                 ...
                 console.log('STEP 2 - CRIT TMP  =  ' + JSON.stringify(component.get("v.criteresTmp"))); 
                 component.set("v.currentStep","2");
    ...
    // Save 
     saveCriteres : function (component, event, helper) {
         ...
         for(var i=0; i<Criteres.length; i++){
             ...
             inputPrincipalCrit.Value=Criteres[i].criterion_response__c;
             inputListCrit.push(inputPrincipalCrit);
             
             for(var l=0; l<Criteres[i].SecondCriteresValues.length; l++){
                 if ( Criteres[i].criterion_response__c==Criteres[i].SecondCriteresValues[l].if_response__c){
         ...
         for ( var i=0; i<Espaces.length; i++){
             ...
             var inputCriteresEspace=[];
                 for ( var l=0; l<Espaces[i].value.length; l++){
                     ...
                     inputOneCrit.Value=Espaces[i].value[l].criterion_response__c;
                 			...
                 			var inputCriteresMoyen=[];
                 			  for ( var j=0; j<Espaces[i].moyens[l].value.length; j++){
                                  ...
                                  inputOneCrit.Id=Espaces[i].moyens[l].value[j].Id;
                     			  inputOneCrit.Value=Espaces[i].moyens[l].value[j].criterion_response__c;
    ...
    CritFormat : function (result){
        console.log('ALlo');
          var Crit = {Id:"", criterion_response__c:"",Name:"", Libelle_du_critere__c:"",
                                 Type_du_critere__c:"",Format_du_critere__c:"",Valeur_par_defaut__c:"", 
                                 mandatory__c:"",display_order__c:"",Offer__c :"",CriteresValues:[], 
                                 Valeur__c:"",Minimum__c:"",Cible__c :"",Maximum__c:"",
                                 Status__c:"",message_Range_Underflow__c:"",message_value_missing__c:"", 
                                 message_Range_Overflow__c :"",SecondCriteresValues:[]
                                };
                     
                     Crit.Id= result.Id;
                     Crit.Name = result.criterion__r.Name; 
                     Crit.Libelle_du_critere__c= result.criterion__r.Libelle_du_critere__c; 
                     Crit.Type_du_critere__c= result.criterion__r.Type_du_critere__c;
                     Crit.Format_du_critere__c= result.criterion__r.Format_du_critere__c; 
                     Crit.Valeur_par_defaut__c= result.criterion__r.Valeur_par_defaut__c; 
                     Crit.mandatory__c= result.mandatory__c;
                     Crit.display_order__c= result.display_order__c; 
                     Crit.Offer__c = result.Offer__c;
                     Crit.Minimum__c = result.Minimum__c;
                     Crit.Maximum__c = result.Maximum__c;
        			 Crit.message_Range_Underflow__c = result.message_Range_Underflow__c;
        			 Crit.message_value_missing__c   = result.message_value_missing__c;
        			 Crit.message_Range_Overflow__c  = result.message_Range_Overflow__c;
                     //Crit.Valeur__c ="";
                     Crit.criterion_response__c  = result.criterion_response__c;
                     Crit.Cible__c = result.criterion__r.Cible__c ;
                     Crit.Status__c = result.Status__c;  
                     

                     (result.Secondary_criterion__r == undefined ?  Crit.SecondCriteresValues = [] : Crit.SecondCriteresValues = result.Secondary_criterion__r)
                     
                     if ( result.criterion__r.Valeur_par_defaut__c!= null){
                         Crit.CriteresValues = result.criterion__r.Valeur_par_defaut__c.split(",");
                     }
        return Crit;
